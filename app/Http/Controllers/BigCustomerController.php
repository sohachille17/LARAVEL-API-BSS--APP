...
use App\Model\BigCustomers;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Validator;


class BigCustomerController extends Controller
{
    //Get all BigCustomers;
    public function getAll(Request $request){
    // the limit is the page number times the number of information per page

    $customers = BigCustomers::all();
    ...
    public function create(Request $request){
        ...
        //$usernameCheck = Customer::where('username',$customerData['accountname'])->first();

        $validateEmail = BigCustomers::where('email',$rules['email'])->first() ;
        if($validateEmail) return response(["message" => "An Account with this email already exist"]);
        $validateUsername = BigCustomers::where('username', $rules['username'])->first();
        if($validateUsername) return response(["message" => "An Account with this username already exist"]);




        //$getCountLength = $rules['telephone1'];
    ...
    */
        ...
        public function updateCustomer(Request $request, $id)
        {
            ...
            if(BigCustomers::where('id', $id)->exists()){
        ...
        {
            $totalCustomers = BigCustomers::all();
